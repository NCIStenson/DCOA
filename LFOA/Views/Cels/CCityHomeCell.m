...
-(void)setModel:(CCityHomeModel *)model {
    ...
    [contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (_posintion == CCityHomeCellPositionLeft) {
            ...
            insets = UIEdgeInsetsMake(.5, 0, .5f, .5f);
        } else if (_posintion == CCityHomeCellPositionCenter){
            
            insets = UIEdgeInsetsMake(.5, 0, .5f, .5f);
        } else if(_posintion == CCityHomeCellPositionRight){
    
            insets = UIEdgeInsetsMake(.5, 0, 0.5f, 0);
        } else {
            
            insets = UIEdgeInsetsMake(.5, 0, 0, .5f);
        }
...
-(void)setIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row > 2) {
        [_contentAllView mas_makeConstraints:^(MASConstraintMaker *make) {
            UIEdgeInsets insets;
            if (_posintion == CCityHomeCellPositionLeft) {
                insets = UIEdgeInsetsMake(0, 0, .5f, .5f);
            } else if (_posintion == CCityHomeCellPositionCenter){
                insets = UIEdgeInsetsMake(0, 0, .5f, .5f);
            } else if(_posintion == CCityHomeCellPositionRight){
                insets = UIEdgeInsetsMake(0, 0, 0.5f, 0);
            } else {
                insets = UIEdgeInsetsMake(.5, 0, 0, .5f);
            }
            make.edges.equalTo(self.contentView).with.insets(insets);
